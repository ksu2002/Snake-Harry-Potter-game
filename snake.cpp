...
#include "game.h"
#include "menu.h"
IMAGE *createmask(IMAGE *p) {// функци€ дл€ создани€ маски
   int w = imagewidth(p);
   int h = imageheight(p);
...
IMAGE *swordBMP;
IMAGE *sword;
IMAGE *tailBMP[4];//изображени€ хвоста
...
void draw_record(void) { //вывод рекорда на экран
   ...
   outtextxy(300, WINDOW_HEIGHT - 60, "–екорд:");
   fclose(record);
}

void draw_points(int points) {// вывод текущего счЄта на экран
   int score = points;
   char buffer[100];
...
int change_direction(int *xhead, int *yhead, int *direction) { //смена направлени€ движени€ змеи
   if (kbhit()) {
      int k=getch();
      if (k==0) k=getch();
      switch (k) {
...
int play_snake_game(void) {
   ...
   int is_food_eaten = 1;
   int is_game_over = 0;
   int pp = 0; // сначала текуща€ видима€ страница 0
   int t = 0; // номер кадра (врем€)
   pole = loadBMP("pole.bmp"); // изображение пол€
...
//маска изображений головы
   ...
   for (int i = 0; i < field_widht; i++)  // цикл по строкам
      free(game_field[i]);   // освобождение пам€ти под строку
   free(game_field);
   free(snake);
   freeimage(pole);
   freeimage(wall);
